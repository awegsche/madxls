...
pub const GENERIC_BUILTINS: Lazy<HashMap<&'static [u8], MadGenericBuilder>> = Lazy::new(|| {
    ...
    insert_generic_builder(&mut builtins, b"value", &[]);
    insert_generic_builder(
        &mut builtins,
        b"use",
        &["sequence", "period", "survey", "range"],
    );
    insert_generic_builder(
        &mut builtins,
        b"select",
    ...
    insert_generic_builder(
        &mut builtins,
        b"setvars",
        &["table", "row", "knob", "const", "noappend"],
    );
    insert_generic_builder(
        &mut builtins,
        b"fill_knob",
        &["table", "row", "knob", "scale"],
    );
    insert_generic_builder(
        &mut builtins,
        b"setvars_lin",
        &["table", "row1", "row2", "param"],
    );
    ...
    insert_generic_builder(
        ...
        b"hkicker",
        &[
            "l", "tilt", "sinkick", "kick", "sintune", "sinpeak", "sinphase",
        ],
    );
    insert_generic_builder(
        &mut builtins,
    ...
    insert_generic_builder(
        ...
        b"crabcavity",
        &[
            "l", "volt", "lag", "freq", "harmon", "rv1", "rv2", "rv3", "rv4", "rph1", "rph2",
            "lagf",
        ],
    );
    insert_generic_builder(
        ...
        b"hacdipole",
        &[
            "l", "volt", "lag", "freq", "ramp1", "ramp2", "ramp3", "ramp4",
        ],
    );
    insert_generic_builder(
        &mut builtins,
    ...
    insert_generic_builder(
        ...
        b"rfmultipole",
        &[
            "l", "volt", "lag", "freq", "harmon", "lrad", "tilt", "knl", "ksl", "pnl", "psl",
        ],
    );
...
impl MadGeneric {
    ...
    pub(crate) fn accept<V: crate::visitor::Visitor>(
        ...
        visitor.visit_generic(self, parser);
        for arg in self.args.iter() {
            if let Some(value) = arg.value.as_ref() {
                value.accept(visitor, parser);
            }
        }
    }
}
        ...
        while let Some(token) = parser.peek_token() {
            ...
            if let Token::Komma(_) = token {
                ...
                if let Some(mut param) = MadParam::parse(parser) {
                    let bytes = &parser
                        .get_element_bytes(&param.attribute)
                        .to_ascii_lowercase()
                        .to_vec();
...
pub fn insert_generic_builder(
    map: &mut HashMap<&'static [u8], MadGenericBuilder>,
...
pub fn insert_generic_builder_known_flags(
    map: &mut HashMap<&'static [u8], MadGenericBuilder>,
    match_name: &'static [u8],
    match_params: &[&str],
    known_flags: &[&[&str]],
